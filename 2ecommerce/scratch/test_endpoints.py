...
sys.path.append('c:/Users/Arch Office/Downloads/2ecommerce')
sys.path.append('c:/Users/Arch Office/Downloads/2ecommerce/front_store')
sys.path.append('c:/Users/Arch Office/Downloads/2ecommerce/admin_dashboard')

import front_store.main as fs_main
import admin_dashboard.main as ad_main

app = FastAPI()
app.include_router(fs_main.app.router)
app.include_router(ad_main.app.router)

calculate_blind_config = fs_main.calculate_blind_config

from front_store.database import SessionLocal, get_db
from front_store.models import Product, Order, OrderItem, StoreSetting, Offer, OrderStatus
...
class TestEcommerceApp(unittest.TestCase):
    ...
    def test_storefront_view(self):
        ...
        self.assertIn("Samrudhi", response.text)
...
if __name__ == "__main__":
    import sys
    with open("c:/Users/Arch Office/Downloads/2ecommerce/scratch/test_output.log", "w", encoding="utf-8") as f:
        sys.stdout = f
        sys.stderr = f
        unittest.main()
